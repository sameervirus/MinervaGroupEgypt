...
class PayMobController extends Controller
{
    ...
    public function checkingOut($orderId)
    {
        ...
        if (property_exists($auth, 'detail')) { // login to PayMob attempt failed.
            return redirect()->back()->with('status', 'Payment faild please try again');
        }
        ...
        $payment_key = PayMob::getPaymentKeyPaymob( // get payment key
            ...
            $paymobOrder->id,
            // For billing data
            $order->email, // optional
            $order->firstname, // optional
            $order->lastname, // optional
            $order->phone // optional
        );

        # code... load view with iframe.
        return view('en.iframe', ['payment_key' => $payment_key]);
    }
    ...
    public function invoice(Request $request)
    {
        ...
        $order = Order::with('item')->where('id', $order_id)->first();
        $data = $request->all();
        return view('en.paymentdone', ['order' => $order, 'page' => 'payment', 'data' => $data]);
    }
